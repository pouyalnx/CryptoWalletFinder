...
def info_thread():
    ...
    while True:
        c, addr = s.accept()    
        c.send(f'ChkedAll:{W1_N} Err:{W1_E} ChkedTr:{W2_N}\n'.encode())
        c.close()
...
logging.basicConfig(filename=BASE_ADDR+"v5_3.log", filemode='w', format='%(name)s - %(levelname)s - %(message)s')
logging.warning('doon')
...
class TRC20:
    def __init__(self,url="https://apilist.tronscan.org/api/account"):
        self.client = Tron()
        self.url=url
...
DB_FILE_NAME=BASE_ADDR+"wallet.db"
...
bep20=BEP20('https://data-seed-prebsc-1-s1.binance.org:8545')
erc20=ERC20("https://rpc.ankr.com/eth")
trc20=TRC20("https://apilist.tronscan.org/api/account")
heco=HECO("https://http-mainnet-node.huobichain.com")
btc=BTC()
...
while True:
    ...
    try:
        if btc.is_have_balance(words) or bep20.is_have_balance(account.address) or erc20.is_have_balance(account.address) or heco.is_have_balance(account.address):
            print("sucessful:",words)
            insert_to_db(words)
            logging.critical(words)
        W1_N+=1
        
        if trc20.is_have_balance(words):
            print("sucessful:",words)
            insert_to_db(words)
            logging.critical(words)
